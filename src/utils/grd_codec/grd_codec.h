...
struct GradientList {
    ...
    void print() const
    {
        for (const auto& e : gradient_list) e.print();
    }
};
...
public:
    ...
    {
        auto red = parseDoubleWithID("Rd  ");
        if (!red) return std::unexpected{red.error()};
        auto green = parseDoubleWithID("Grn ");
        if (!green) return std::unexpected{green.error()};
        auto blue = parseDoubleWithID("Bl  ");
    ...
    {
        auto cyan = parseDoubleWithID("Cyn ");
        if (!cyan) return std::unexpected{cyan.error()};
        auto magenta = parseDoubleWithID("Mgnt");
        if (!magenta) return std::unexpected{magenta.error()};
        auto yellow = parseDoubleWithID("Ylw ");
        if (!yellow) return std::unexpected{yellow.error()};
        auto black = parseDoubleWithID("Blck");
        if (!black) return std::unexpected{black.error()};
        return CMYC{cyan.value(), magenta.value(), yellow.value(), black.value()};
    }

    std::expected<Grsc, std::string> parseGrsc()
    {
        auto gray = parseDoubleWithID("Gry ");
        if (!gray) return std::unexpected{gray.error()};
        return Grsc{gray.value()};
    }

    std::expected<HSBC, std::string> parseHSBC()
    {
        auto hue = parseUnitDoubleWithID("#Ang", "H   ");
        if (!hue) return std::unexpected{hue.error()};
        auto saturate = parseDoubleWithID("Strt");
        if (!saturate) return std::unexpected{saturate.error()};
        auto brightness = parseDoubleWithID("Brgh");
        if (!brightness) return std::unexpected{brightness.error()};
        return HSBC{hue.value(), saturate.value(), brightness.value()};
    }

    std::expected<LbCl, std::string> parseLbCl()
    {
        auto luminance = parseDoubleWithID("Lmnc");
        if (!luminance) return std::unexpected{luminance.error()};
        auto a = parseDoubleWithID("A   ");
        if (!a) return std::unexpected{a.error()};
        auto b = parseDoubleWithID("B   ");
    ...
    std::expected<BookColor, std::string> parseBkCl()
    {
        ...
        auto book_name = parseUnicodeString();
        if (!book_name) return std::unexpected{book_name.error()};
        auto color_name = parseName();
        if (!color_name) return std::unexpected{color_name.error()};
        if (auto err = expectID("bookID"); !err) return std::unexpected{err.error()};
        auto book_id = parseInt32();
        if (!book_id) return std::unexpected{book_id.error()};
        if (auto err = expectID("bookKey"); !err) return std::unexpected{err.error()};
        auto book_key = parseRawData();
        if (!book_key) return std::unexpected{book_key.error()};
    ...
    std::expected<TransparencyStopObject, std::string> parseTransparencyStopObject()
    {
        ...
        auto opacity = parseUnitDoubleWithID("#Prc", "Opct");
        if (!opacity) return std::unexpected{opacity.error()};
        auto location = parseInt32WithID("Lctn");
        if (!location) return std::unexpected{location.error()};
        auto midpoint = parseInt32WithID("Mdpn");
        if (!midpoint) return std::unexpected{midpoint.error()};
...
#define TRY(expr)                                         \
    if (auto _err = (expr); !_err) return std::unexpected \
        {                                                 \
            _err.error()                                  \
        }
